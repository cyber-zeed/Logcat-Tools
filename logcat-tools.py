...
class AdbLogcatApp:
    def __init__(self, root):
        self.root = root
        self.root.title("Logcat Tools 0.1")
        ...
        self.set_verbose_button = tk.Button(root, text="Set Logcat to Verbose", command=self.set_logcat_to_verbose)
        self.set_verbose_button.grid(row=3, column=0, padx=10, pady=5, sticky="nw")

        self.set_logcat_size_button = tk.Button(root, text="Set Logcat Size", command=self.set_logcat_size)
        self.set_logcat_size_button.grid(row=4, column=0, padx=10, pady=5, sticky="nw")

        self.reboot_button = tk.Button(root, text="Reboot", command=self.reboot_device)
        self.reboot_button.grid(row=5, column=0, padx=10, pady=5, sticky="nw")

        self.clear_logcat_button = tk.Button(root, text="Clear Logcat", command=self.clear_logcat)
        self.clear_logcat_button.grid(row=6, column=0, padx=10, pady=5, sticky="nw")

        self.stop_button = tk.Button(root, text="Stop", command=self.stop_logging)
        self.stop_button.grid(row=7, column=0, padx=10, pady=5, sticky="nw")

        self.stop_and_save_button = tk.Button(root, text="Stop and Save Logs", command=self.stop_and_save_logs)
        self.stop_and_save_button.grid(row=8, column=0, padx=10, pady=5, sticky="nw")

        self.change_theme_button = tk.Button(root, text="Change Theme", command=self.change_theme)
        self.change_theme_button.grid(row=9, column=0, padx=10, pady=5, sticky="nw")
    ...
    def stop_and_save_logs(self):
        ...
        messagebox.showinfo("Logs Saved", f"Logs saved to {log_file_name}")
    ...
    def set_logcat_to_verbose(self):
        subprocess.run(['adb', 'shell', 'setprop', 'persist.log.tag', 'V'])
        messagebox.showinfo("Set Logcat to Verbose", "Logcat has been set to verbose.")
